...
import { Loader2, AlertCircle, Star, ArrowRight, UserCheck } from 'lucide-react';

import { GoogleIcon, GithubIcon } from '@/components/icons/BrandIcons';

export default function SignIn() {
  const router = useRouter();
  const { signInWithEmail, signInWithGoogle, signInWithGithub, isConfigured } = useAuth();
      ...
      <div className="max-w-md w-full mx-auto my-auto py-8">
        <div className="bg-white/75 backdrop-blur-2xl border border-white/90 shadow-2xl shadow-blue-500/10 rounded-3xl p-8 sm:p-10 space-y-7">
          ...
          <div className="space-y-2 text-center">
            <div className="inline-flex items-center gap-1.5 text-amber-700 text-xs font-extrabold bg-amber-500/10 border border-amber-500/30 px-3.5 py-1 rounded-full backdrop-blur-md">
              <Star size={13} className="fill-amber-500 stroke-amber-500" />
              <span>★ 4.96 stars across 5,000+ reviews</span>
            </div>
            ...
            <p className="text-slate-500 text-xs sm:text-sm">
              Welcome back! Access your video projects and studio editor.
            </p>
          </div>
          ...
          {!isConfigured && (
            <div className="p-3.5 bg-amber-500/10 border border-amber-500/30 rounded-2xl text-amber-800 text-xs flex items-center gap-2">
              ...
              <span>Firebase Auth keys not configured.</span>
            </div>
          )}
          ...
          <form onSubmit={handleSignIn} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-bold text-sm shadow-lg shadow-blue-500/25 transition-all hover:scale-[1.01] flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {loading ? (
