...
const FontSelectionModal = dynamic(() => import('@/components/FontSelectionModal').then(m => m.FontSelectionModal), { ssr: false });
const AuthRequiredModal = dynamic(() => import('@/components/AuthRequiredModal').then(m => m.AuthRequiredModal), { ssr: false });
...
function getActiveChunkFromPhrases(phrases: CaptionPhrase[], currentTime: number): CaptionWord[] {
...
function drawCaptionsToCanvas(
  ...
  const activeWords = getActiveChunkFromPhrases(phrases, currentTime);
  if (activeWords.length === 0) return;

  const activeWordObj = activeWords.find((w: CaptionWord) => currentTime >= w.start && currentTime <= w.end) || activeWords[0];
  ...
  const wordItems = activeWords.map((w: CaptionWord) => {
  ...
  const gap = 12 * scale;
  const totalWidth = wordItems.reduce((acc: number, item: any) => acc + item.width, 0) + (wordItems.length - 1) * gap;
...
function EditorStudio() {
  ...
  const phrases = useMemo(() => buildPhrases(words), [words]);
  const activeChunk = useMemo(() => getActiveChunkFromPhrases(phrases, currentTime), [phrases, currentTime]);
  const activeWordObj = useMemo(
    () => activeChunk.find((w: CaptionWord) => currentTime >= w.start && currentTime <= w.end) || activeChunk[0],
    [activeChunk, currentTime]
  );

  const filteredPhrases = useMemo(
    () =>
      phrases.filter((p: CaptionPhrase) =>
        clipSearch ? p.words.some((w: CaptionWord) => w.word.toLowerCase().includes(clipSearch.toLowerCase())) : true
      ),
    [phrases, clipSearch]
  );
    ...
    <div className="flex-1 flex flex-col h-full bg-white/75 backdrop-blur-xl overflow-hidden text-slate-900">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {filteredPhrases.length === 0 ? (
          ...
          filteredPhrases.map((phrase: CaptionPhrase, pIdx: number) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {phrase.words.map((wObj: CaptionWord, wIdx: number) => {
  ...
  return (
    <div className="h-[100dvh] bg-gradient-to-b from-slate-50 via-blue-50/40 to-white text-slate-900 flex flex-col overflow-hidden selection:bg-blue-500 selection:text-white relative font-sans antialiased">
                  ...
                  {activeChunk.length > 0 && (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-center gap-2">
                        {activeChunk.map((w: CaptionWord, idx: number) => {
                          const isActive =
      ...
      <FontSelectionModal
        ...
        onSelectFont={(font: string) => setStyles((prev) => ({ ...prev, fontFamily: font }))}
      />
