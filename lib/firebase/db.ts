...
const projectCacheMap = new Map<string, { data: Project; timestamp: number }>();
...
export async function fetchFirebaseProject(id: string): Promise<Project | null> {
  const cached = projectCacheMap.get(id);
  ...
  if (isFirebaseConfigured() && db) {
    try {
      ...
      if (snapshot.exists()) {
        ...
        projectCacheMap.set(id, { data: projectData, timestamp: Date.now() });
        return projectData;
      }
    } catch {
      // fallback
  ...
  if (typeof window !== 'undefined') {
    ...
    if (savedRaw) {
      ...
      if (found) {
        projectCacheMap.set(id, { data: found, timestamp: Date.now() });
      }
      return found;
    }
  }
